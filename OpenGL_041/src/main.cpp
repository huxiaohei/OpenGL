...
int main(int argc, char *argv[]) {
    GLFWwindow *frameWindow = createWindow("反射", SCREEN_WIDTH, SCREEN_HEIGHT);
    if (frameWindow == NULL) {
...
int draw(GLFWwindow *window) {
    ...
    boxShader->setVertexAttributePointer("texcoordPos", 2, GL_FLOAT, GL_FALSE, sizeof(GLfloat) * 8, (void *)(sizeof(GLfloat) * 3));
    boxShader->setVertexAttributePointer("normal", 3, GL_FLOAT, GL_FALSE, sizeof(GLfloat) * 8, (void *)(sizeof(GLfloat) * 5));
    ...
    GLuint boxTexture = loadImage("res/box.jpg");
    GLuint boxBorderTexture = loadImage("res/box_border.png");
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        boxShader->setUniformIntByName("isReflect", 0);
        ...
        boxShader->setUniformMatrix4fvByName("model", 1, GL_FALSE, glm::value_ptr(model));
        boxShader->setUniformIntByName("isReflect", 1);
        boxShader->setUniformIntByName("skybox", 1);
        boxShader->setUniformFloatVec3ByName("cameraPos", cameraPos);
        glBindTexture(GL_TEXTURE_2D, boxBorderTexture);
    ...
    glDeleteTextures(1, &boxBorderTexture);
